...
import { inventoryService, authService, aiService, leadService, negotiationService, analytics } from "./services.js";
import { vehicleCard, vehicleArt, chip, eyebrow, esc } from "./ui.js";
import { searchFunnel } from "./domain.js";
import {
...
const SHEET_ROWS = [
  ...
  ["color", "Color"], ["location", "Location"], ["radius", "Radius"],
];
    ...
    <div class="dp-intake-grid">
      <section class="dp-chat">
        ...
        ${step ? `<div class="dp-answer">
          ${step.layout === "grid" && opts.length > 14 ? `
            <input class="dp-input dp-picker-search" id="picker-search" autocomplete="off"
              placeholder="Filter ${opts.length} options…" value="${esc(state.pickerQuery)}"
              data-action="picker-filter" />` : ""}
          <div class="${step.layout === "grid" ? "dp-optgrid" : "dp-chips"}">${opts
            .filter((o) => !state.pickerQuery || o.label.toLowerCase().includes(state.pickerQuery.toLowerCase()))
            .map((o) => chip(o.label, {
